...
function Surveyor(tests, options) {
    ...
    this.count = options.count || 1000; // Should this match left? At least to start?
    this.running = false;

    this.tests = tests;
    this.names = Object.keys(tests);
    this.left = this.count * this.names.length;
...
function next() {
    ...
    self.sampleTest(self.tests[name], name, function testComplete() {
        ...
        if (self.verbose) {
            // TODO: Generate from event in bin/survey.
            console.log('Running ' + name + '. ' + self.left + ' left.');
        }
